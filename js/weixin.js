...
var scenesIndex = 0;
var scenesFn = [
  builderSceneHistogram,
  builderScenePieChart,
  builderSceneTextRect
];
// ======  =>S 监听 滑动，切换场景的事件处理=============
//监听  开始时间，记录当前鼠标的方向起点
function addSceneChangeEvent() {
  ...
  stage.on("contentTouchend contentMouseup", function(e) {
    if (stageEndY > stageStartY) {
      //下滑
      scenesIndex = scenesIndex - 1 >= 0 ? scenesIndex - 1 : 0;
    } else {
      //上滑
      scenesIndex =
        scenesIndex + 1 >= scenesFn.length ? scenesIndex : scenesIndex + 1;
    }
...
function builderScenePieChart() {
  ...
  var pieChart = new PieChart({
    ...
    x: stage.width() / 2,
    y: stage.height() / 2,
    radius: 0.3 * maxSize, //半径
    txtAwayFromWedge: 0.3 * 0.3 * maxSize //扇形上的文字的距离圆形的距离
  });
...
// var s3 = new WebScence({

// });
// var s4 = new WebScence({
