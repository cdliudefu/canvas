//场景
function WebScence(options) {
  this.stage = options.stage;
  this.init = options.init || WebScence.voidFn;
  this.pre = options.pre || WebScence.voidFn;
  this.post = options.post || WebScence.voidFn;
...
WebScence.prototype = {
  ...
  play: function() {
    ...
    var doPre = function doPre() {
      ...
      WebScence.currentScence = _this;
      _this.pre();
    };
    if (WebScence.currentScence) {
      // 执行上一个场景的出场动画
      WebScence.currentScence.post(doPre);
    } else {
      doPre(); //执行当前入场动画
    }
  }
